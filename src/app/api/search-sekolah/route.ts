...
export async function GET(request: Request) {
  try {
    ...
    console.log('🔍 Search API called with:', { query, limit });
    ...
    try {
      ...
      const searchUrl = `${supabaseUrl}/rest/v1/sekolah?select=id,nama_sekolah,latitud,longitud&nama_sekolah=ilike.*${encodeURIComponent(query)}*&order=nama_sekolah.asc&limit=${limit}`;
      
      console.log('🔗 Direct API call to:', searchUrl.substring(0, 100) + '...');
      ...
      console.log(`✅ Found ${data?.length || 0} matching schools via direct API`);
      ...
      try {
        ...
        console.log(`✅ Found ${data?.length || 0} matching schools via Supabase client fallback`);
    ...
    console.error('💥 Server error in search-sekolah:', error);
    return NextResponse.json(
      { 
